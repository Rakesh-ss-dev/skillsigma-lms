...
export default function CoursePlayer() {
    ...
    async function fetchData() {
      try {
        ...
        if (uiData.curriculum && uiData.curriculum.length > 0) {
          ...
          const firstUnfinishedItem = uiData.curriculum.find(
            (item) => !item.completed,
          );
  ...
  const handleCompletion = async (itemId: number) => {
    ...
    const currentIndex = courseData.curriculum.findIndex(
      (i) => i.id === itemId,
    );
    ...
    const newProgress = Math.round(
      (completedCount / updatedCurriculum.length) * 100,
    );
  ...
  return (
    <>
      <PageMeta title={courseData.title} description={courseData.title} />
      <div className="flex h-screen w-full bg-white dark:bg-gray-950 overflow-hidden">
        {/* MAIN CONTENT AREA */}
        <main
          className={`flex-1 flex flex-col h-full transition-all duration-300 ease-in-out relative ${sidebarOpen ? "lg:mr-90" : ""}`}
        >
          {/* HEADER */}
            ...
            {isFinished ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-full p-6 text-center">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-8 max-w-sm">
                  ...
                  <strong>{courseData.title}</strong>. Great job on reaching the
                  finish line!
                </p>
