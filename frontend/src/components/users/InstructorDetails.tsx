...
const InstructorDetails = ({ instructor }: any) => {
  ...
  const assignCourse = async () => {
    try {
      ...
      await API.post(`/courses/${selectedCourse}/add_instructor/`, {
        ...formdata,
      });
  ...
  const removeCourse = async (courseId: any) => {
    try {
      const formdata = { instructor_id: instructor.id };
      await API.post(`/courses/${courseId}/remove_instructor/`, {
        ...formdata,
      });
  ...
  return (
    <>
      ...
      />
      <div>
        <Link className="text-brand-500 mb-5 flex gap-3" to="/instructors">
          <ArrowBigLeftDash /> Back to Instructors List
        </Link>
              ...
              {assignedCourses.length !== 0 ? (
                ...
                <div>
                  ...
                  <div className="flex justify-end">
                    <Button onClick={assignCourse} className="mt-3">
                      Assign
                    </Button>
                  </div>
                </div>
